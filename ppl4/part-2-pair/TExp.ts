...
const parseCompoundTExp = (texps: any[]): PairTExp | ProcTExp | Error => {
    const pair_index = texps.indexOf('Pair');
    if(texps.length == 3 && pair_index == 0) {
        const left = parseTExp(texps[1]);
        const right = parseTExp(texps[2]);
        if(isError(left) || isError(right)) return Error('Parse failed');
        /*if(isTupleTExp(left) || isTupleTExp(right)) return Error('invalid pair');*/ //TODO CHECK  
        return makePairTExp(left,right);
    }
...
const matchTVarsinPairs = <T1, T2>(te1: TExp, te2: TExp,
                            succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
                            fail: () => T2): T1 | T2 => 
    (isPairTExp(te1) && isPairTExp(te2)) ? matchTVarsInTEs([te1.left, te1.right], [te2.left, te2.right], succ, fail) : fail();

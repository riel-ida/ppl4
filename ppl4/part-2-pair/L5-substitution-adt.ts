...
import { eqTVar, isAtomicTExp, isProcTExp, isTVar, makeProcTExp, unparseTExp, TExp, TVar, isPairTExp, isTupleTExp, makePairTExp } from "./TExp";
...
export const checkNoOccurrence = (tvar: TVar, te: TExp): true | Error => {
    const check = (e: TExp): true | Error => {
        if(isPairTExp(e)) {
            if(!isError(check(e.left)) && !isError(check(e.right))) return true; 
            return Error('invalid PairTExp');
        }
...
export const applySub = (sub: Sub, te: TExp): TExp =>{
    if(isPairTExp(te) && !isEmptySub(sub)) {
        let left = applySub(sub,te.left);
        let right = applySub(sub,te.right);
        if(!isTupleTExp(left) && !isTupleTExp(right)) return makePairTExp(left,right);
        else Error('invalid Pair');
    }
    return isEmptySub(sub) ? te :
    isAtomicTExp(te) ? te :

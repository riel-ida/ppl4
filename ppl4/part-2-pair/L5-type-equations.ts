...
export const makeEquationFromExp = (exp: A.Exp, pool: Pool): Equation[] =>
    ...
    //Pair case 
    A.isPair(exp) ? [makeEquation(inPool(pool, exp), T.makePairTExp(inPool(pool,exp.left),inPool(pool,exp.right)))] :
...
const splitEquation = (eq: Equation): Equation[] => { //TODO MAYBE EXPORT ?
    if(T.isPairTExp(eq.left) && T.isPairTExp(eq.right)) {
        return [makeEquation(eq.left.left, eq.right.left), makeEquation(eq.left.right, eq.right.right)];
    }
    return (T.isProcTExp(eq.left) && T.isProcTExp(eq.right)) ?

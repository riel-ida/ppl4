...
export type Pair = {tag: "Pair", left: CExp, right: CExp};
export const makePair = (left: CExp, right: CExp): Pair => ({tag:"Pair", left: left, right: right})
export const isPair = (x:any): x is Pair=> x.tag === "Pair";
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | Pair;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isPair(x);
...
const parseCompound = (sexps: any[]): Parsed | Error =>
    ...
    first(sexps) === "quote" ? parsePairExp(sexps) :
    parseCExp(sexps);

const parsePairExp = (sexp: any[]) : Pair | Error =>{
    if(sexp[1].length != 3 || sexp[1][1] != ".") return Error('Invalid Pair expression');
    const left = parseCExp(sexp[1][0]);
    const right = parseCExp(sexp[1][2]);
    if(isError(left) || isError(right)) return Error('Parse failed');
...
export const unparse = (e: Parsed | Error): string | Error =>
    ...
    isPair(e)? `'(${unparse(e.left)} . ${unparse(e.right)})` :

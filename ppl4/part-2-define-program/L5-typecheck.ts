...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    const dec_TExp = exp.var.texp;
    const val_TExp = typeofExp(exp.val, tenv);
    if(isError(checkEqualType(dec_TExp, val_TExp, exp))) return Error("invalid DefineExp");
    return makeVoidTExp();
};
...
export const typeofProgram = (exp: Program, tenv: TEnv): TExp | Error => {
    if(isEmpty(exp)) return Error("invalid program");
    if(isEmpty(rest(exp.exps))) return typeofExp(first(exp.exps), tenv);
    if(isError(typeofExp(first(exp.exps), tenv))) return typeofExp(first(exp.exps), tenv);
    if(isDefineExp(typeofExp(first(exp.exps), tenv))) return defineExpinProgram(exp, tenv);
    return typeofProgram(makeProgram(rest(exp.exps)),tenv);
};

export const defineExpinProgram = (exp: Program, tenv: TEnv): TExp | Error => {
    if(isError(typeofDefine(first(exp.exps), tenv))) return Error("invalid declaration");
    const extendedTEnv = makeExtendTEnv([first(exp.exps).var.var], [first(exp.exps).var.texp], tenv);
    return typeofProgram(makeProgram(rest(exp.exps)), extendedTEnv);
}
